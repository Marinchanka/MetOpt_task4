# Лабораторная работа №4. Решение задачи динамического программирования
...
Критерий оптимальности — максимизация математического ожидания финальной выплаты:
...
- максимум берётся по всем допустимым действиям,

- ожидание берётся по всем возможным сценариям следующего шага,

- функция перехода формирует состояние на следующем этапе.

Граничное условие:

$V_T(s_T) = F(s_T)$

где $F$— конечная функция выгоды (например, итоговая стоимость портфеля).

Конечное условие:

$V_3(s_3) = x^1_3 + x^2_3 + d_3 + c_3.$

### Обратное прохождение (backward pass)

Обратное прохождение — основной вычислительный этап метода динамического программирования.

Идея:
мы начинаем с последнего этапа, где значение функции $V_T(s_T)$ известно, и движемся назад, вычисляя оптимальные значения для более ранних этапов.

Пошагово:

1. Для всех состояний $s_T$:
   
$$$V_T(s_T) = F(s_T)$$$

2. Для всех $t = T -1, T-2,...,0:$
для каждого состояния $s_t$:
   - перебираем все действия $a_t$,
   - считаем ожидаемый выигрыш,
   - выбираем максимальный:
     
     $$$V_t(s_t) = \max_{a_t} \sum_{w} p_t(w) \, V_{t+1}( f(s_t, a_t, w) )$$$
   - сохраняем оптимальное действие:
     
     $$$\pi_t(s_t)=arg \max_{a_t}(.)$$$

Результат:
- вычислены все значения $V_t$,
- построена оптимальная стратегия $\pi_t$ для каждого состояния.

### Прямое прохождение (forward pass)

Прямое прохождение используется только после выполнения обратного, чтобы получить явную оптимальную траекторию.

Идея:

Начав из начального состояния, мы просто выполняем заранее найденные оптимальные действия.

Пошагово:

1. Берём начальное состояние$s_0$.
2. На шаге $t$:
  - выбираем действие по стратегии:
    
$$$a_t=\pi_t(s_t)$$$

  - выполняем действие и реализуем сценарий,
  - переходим в новое состояние:
    
$$$s_t+1 = f(s_t, a_t, w_t)$$$

3. Повторяем до этапа $T$.

Результат:
- конкретная траектория оптимального управления,
- удобна для демонстрации и проверки работы программы.
